const CFG = { id: '2523c510-9ff0-415b-9582-93949bfae7e3', chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnQr: 4, upPack: 20 * 1024, maxED: 8 * 1024, concur: 4, proxyIP: '' };
export default { fetch: (req, env = {}) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new Response('GrainTCP ready') }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
...
const proxyList = raw => String(raw || '').split(',').map(s => s.trim()).filter(Boolean).map(v => { let h = v, p = 443; if (v.startsWith('[')) { const m = /^\[([^\]]+)\](?::(\d+))?$/.exec(v); if (!m) return null; h = m[1]; p = +(m[2] || 443); } else { const i = v.lastIndexOf(':'); if (i > -1 && /^\d+$/.test(v.slice(i + 1))) { h = v.slice(0, i); p = +v.slice(i + 1); } } return h && p > 0 && p < 65536 ? { h, p } : null; }).filter(Boolean);
const dial = async (f, h, p, rawProxy) => { try { return await raceSprout(f, h, p); } catch (e) { const ps = proxyList(rawProxy); if (!ps.length) throw e; let last = e; for (let i = 0; i < ps.length; i += CFG.concur) { try { const batch = ps.slice(i, i + CFG.concur), ts = batch.map(x => sprout(f, x.h, x.p)); const w = await Promise.any(ts); ts.forEach(t => t.then(s => s !== w && s.close(), () => {})); return w; } catch (x) { last = x; } } throw last; } };
...
const mill = async (rd, w) => { let r, byob = 1; try { r = rd.getReader({ mode: 'byob' }); } catch { r = rd.getReader(); byob = 0; } const tx = mkDn(w); let buf = new ArrayBuffer(CFG.chunk);
  try { for (;;) { const x = byob ? await r.read(new Uint8Array(buf, 0, CFG.chunk)) : await r.read(), { done, value: v } = x; if (done) break; if (!v?.byteLength) continue; if (v.byteLength >= (CFG.chunk >> 1)) tx.reap(), w.send(v), buf = new ArrayBuffer(CFG.chunk); else tx.send(v.slice()), byob && (buf = v.buffer.byteLength >= CFG.chunk ? v.buffer : new ArrayBuffer(CFG.chunk)); } tx.reap(); } catch {} finally { try { tx.reap(); } catch {} try { r.releaseLock(); } catch {} } };
const ws = async (req, env) => {
  const [client, server] = Object.values(new WebSocketPair()); server.accept({ allowHalfOpen: true }); server.binaryType = 'arraybuffer'; const fetcher = req.fetcher, rawProxy = env.PROXYIP || CFG.proxyIP;
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    if (closed) break; if (!sock) { const [d] = uq.bundle(); if (!d) break; const r = relay(d); if (!r) throw wither(); server.send(new Uint8Array([d[0], 0])); const host = addr(r.addrType, r.targetAddrBytes), port = r.port, payload = d.subarray(r.dataOffset); sock = await dial(fetcher, host, port, rawProxy); if (!sock) throw wither(); curW = sock.writable.getWriter(); const [first] = uq.bundle(payload); first?.byteLength && await curW.write(first); mill(sock.readable, server).finally(() => wither()); continue; }
